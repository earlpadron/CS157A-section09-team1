    ...
    <% 
     String db = "cs157a"; //change to db name
        ...
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT * FROM proj1test.rooms WHERE availability = 'available'"); //change to schema name.table name
            %>
            <form method='post' action ='roomReservation' >
            <table>
      		<thead>
      			<tr>
      				<td>Room Number</td>
            ...
            <%
            rs = stmt.executeQuery("SELECT * FROM proj1test.rooms WHERE availability = 'available'");
            while(rs.next())
            {
            	%>
